...
combined_sources = cell(1,6);
combined_gms = cell(1,6);
...
for j = 1:12
    ...
    [~,idxes]= sort(tmp_accs);
    new_num = floor((12-1)/2);
    for i = 1:new_num
        mean_tmp_acc = mean(tmp_accs);
        new_source = data_sources{i};
        new_gm = gms{i};
        new_weight = 0;
        idx_list = [1:j-1 j+1:12];
        tmp_source = data_sources{idx_list(end-i+1)};
        tmp_gm = gms{idx_list(end-i+1)};
        for ii = 1:2
        ...
        combined_sources{i} = new_source;
        combined_gms{i} = new_gm;
    end
    combined_sources{6} = data_sources{idx_list(6)};
    combined_gms{6} = gms{idx_list(6)};
    
    for i = 1:6
        %% training CSP using new covariance matrix. data_source is useless
        [ csp_coeff3,all_coeff3] = csp_analysis(combined_sources{i},9,csp_per_class, 0,combined_gms{i});
        [ data_source_filter3 ] = csp_filtering(combined_sources{i}, csp_coeff3);
        ...
        for idx = 1:2
            for idx2 = 1:size(data_target_log3{idx},2)
        ...
        [X_LDA, predicted_y_class3] = lda_apply(Xproj3(size_xproj3+1:end,:), W3, B3);
        X_LDAs = cell(1,6);
        predicted_y_classes{i} = predicted_y_class3;
        estimate_performance = zeros(size(y_test_t,1),1);
        for k = 1:6
            if (k == i)
                continue;
            ...
            %% training CSP using new covariance matrix. data_source is useless
            [ csp_coeff2,all_coeff2] = csp_analysis(combined_sources{k},9,csp_per_class, 0,combined_gms{i});
            [ data_source_filter2 ] = csp_filtering(combined_sources{k}, csp_coeff2);
