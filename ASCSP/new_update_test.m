% new_update_test.m
function [result,store_idx] = new_update_test(gm,X1_s,X2_s,X1_t,X2_t)
result=gm;
...
%     end
    %left
    result_tmp{1} = result_tmp{1}*(num_select-weight)/(num_select+1) + C_new*(1+weight)/(num_select+1);
    for j = (i+1):n_target*2
        ...
        result_tmp_1{2} = result_tmp{2}*(num_select-weight)/(num_select+1) + C_new*(1+weight)/(num_select+1);
...
%         C_new = cov(squeeze(X1_t(sampleIdx_left,:,:))')/trace(cov(squeeze(X1_t(sampleIdx_left,:,:))'));
        result{1} = result{1}*(num_select-weight)/(1+num_select) + C_new*(1+weight)/(1+num_select);
        if labelIdx_right == 1
...
%         C_new = cov(squeeze(X2_t(sampleIdx_right,:,:))')/trace(cov(squeeze(X2_t(sampleIdx_right,:,:))'));
        result{2} = result{2}*(num_select-weight)/(1+num_select) + C_new*(1+weight)/(1+num_select);
