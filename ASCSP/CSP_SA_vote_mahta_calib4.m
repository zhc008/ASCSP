...
for i = 1:12
    ...
    for j= [1:i-1 i+1:12]
        ...
        name2 = ['../Mahta_data/S',int2str(j),'_calib.mat'];
        load(name2)
        
%         num_aL_ol = size(aL_calib,3);
%         num_tL_ol = size(tL_calib,3);
%         num_aR_ol = size(aR_calib,3);
%         num_tR_ol = size(tR_calib,3);
        num_aL_ol = floor(size(aL_calib,3)/2);
        num_tL_ol = floor(size(tL_calib,3)/2);
        num_aR_ol = floor(size(aR_calib,3)/2);
        num_tR_ol = floor(size(tR_calib,3)/2);
        ...
        for target_trial = 1:num_aL_ol       
            data_target{1}{target_trial} = squeeze(aL_calib(:,:,target_trial));
        end
        for target_trial = 1:num_tL_ol
            data_target{1}{target_trial+num_aL_ol} = squeeze(tL_calib(:,:,target_trial));
        end
        
        % right trials
        for target_trial = 1:num_aR_ol       
            data_target{2}{target_trial} = squeeze(aR_calib(:,:,target_trial));
        end
        for target_trial = 1:num_tR_ol
            data_target{2}{target_trial+num_aR_ol} = squeeze(tR_calib(:,:,target_trial));
        end
        ...
        left_size = size(data_target_log{1},2);

%         assert(mod(left_size,2) == 0, 'Size mismatches');
        
        [~, predicted_y_class1] = lda_apply(Xproj(size_xproj+1:size_xproj+left_size,:), W, B);
        predicted_y_class1(predicted_y_class1 == 1) = 0;   % incorrect choice
        predicted_y_class1(predicted_y_class1 == -1) = 1;
        [X_LDA, predicted_y_class2] = lda_apply(Xproj(size_xproj+left_size+1:end,:), W, B);    % there is a vector output! should all be -1! 
...
mean_all = mean(nonzeros(acc_for_all));
disp(mean_all)
mean_rows = zeros(1,12);
for i = 1:12
   mean_rows(i) = mean(nonzeros(acc_for_all(:,i)));
end
...
for j = 1:12
    ...
    [~,idx]= sort(tmp_accs);
    mean_acc = mean(tmp_accs);
    if mean_rows(j) < mean_all
        index = idx(end);
%     elseif mean_rows(j) >= mean_all && mean_rows(j) < 0.6
%         index = idx(2);
    else
        index = idx(end);
    end
    for i = [1:j-1 j+1:12]
        ...
        predicted_ys = predicted_ys + tmp_accs(i) * predicted_y_classes{i};
        denominator = denominator + tmp_accs(i);
    end
