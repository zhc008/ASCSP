...
%     fprintf(fileID,'Tain ID %d\n',sub_idx);
    ...
    name = ['data1/S',int2str(sub_idx), '_I1_allFreqs_U.mat'];
    load(name);
...
%     num_tmp = size(train_Bl,2);
    ...
    for idx = 1:num_tmp
        ...
        X1_temp = squeeze(train_Gl(freqs_idx,idx,:,:) );
        ...
        X2_temp = squeeze(train_Gr(freqs_idx,idx,:,:) );
    ...
    for idx = 1:num_tmp
        ...
        % left data both good and bad with instance 1
        X1_temp = squeeze(train_Bl(freqs_idx,idx,:,:) );
        data_source{1}{idx+num_tmp} = X1_temp;
        cov1_temp = cov(X1_temp.');
        sigma1 = sigma1 + cov1_temp/trace(cov1_temp);
        
        %data_source{1}{i} = [reshape( train_Gl(freqs_idx,i), [] ) ; train_Bl()];
        X2_temp = squeeze(train_Br(freqs_idx,idx,:,:) );
    ...
    for target_idx= [1:sub_idx-1 sub_idx+1:6]
        ...
        name = ['data1/S',int2str(target_idx), '_I1_allFreqs_U.mat'];
        load(name)
...
%         gm{1} = squeeze(cm(sub_idx, 1, freqs_idx, 1, :,:));
        ...
        [gm1,store_idx] = update_v1_simplify(gm,data_source,data_target);
        
        %% training CSP using new covariance matrix. data_source is useless
%         gm1{1} = gm1{1} + c*eye(size(X1_s,2));
%         gm1{2} = gm1{2} + c*eye(size(X1_s,2));
        ...
        text = [sub_idx,' to ',target_idx];
        fprintf(text)
        fprintf(': %f\n', acc2)
        acc_for_all(sub_idx,target_idx) = acc2;
    end
end