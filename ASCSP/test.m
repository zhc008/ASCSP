...
%        disp(cv_error)
    ...
    c = alpha(best_j);
    gm_t = cell(1,2);
    gm_t{1} = cov_s1 + c*eye(size(X1_s,2));
    gm_t{2} = cov_s2 + c*eye(size(X2_s,2));
    for j = 1:5
        if j == i
           continue; 
        end
        ...
        %% TRCSP
        [ csp_coeff,all_coeff] = csp_analysis(data_source,9,csp_per_class, 0,gm_t);
        [ data_source_filter ] = csp_filtering(data_source, csp_coeff);
