% update_selective2.m
function [result,store_idx] = update_selective2(gm,data_source,data_target)
csp_per_class=3;
result=gm;
...
for i = 1:n_target
    ...
    for j = (i+1):n_target
        ...
        Xproj_class1 = zeros(0,2);
        Xproj_class2 = zeros(0,2);
        if total ~= 0
            Xproj_class1 = Xproj(2*n_source+store_idx(:,1),:);
        
            Xproj_class2 = Xproj(2*n_source+store_idx(:,2),:);
        end
        Xproj_class1 = [Xproj_class1;Xproj(2*n_source+i,:)];
        Xproj_class2 = [Xproj_class2;Xproj(2*n_source+j,:)];
        trainY3 = [(-1)*ones(size(Xproj_class1,1),1); ones(size(Xproj_class2,1),1)];
        Xproj_train = [Xproj_class1; Xproj_class2];
        
        %% lda
        [W2,B2,~] = lda_train_reg(Xproj_train, trainY3, 0);
        [~, predicted_y1] = lda_apply(Xproj(1:2*n_source,:), W2, B2);
        trueY = [(-1)*ones(n_source,1); ones(n_source,1)];
        test_err = sum(abs(trueY - predicted_y1))/(2*n_source);
        valid_acc = 1-test_err/2;
