...
for instance = 1:10
    ...
    num_aL = size(aL_test,1);
    num_tL = size(tL_test,1);
    num_aR = size(aR_test,1);
    num_tR = size(tR_test,1);
    
    num_aL = size(aL_test,1) + size(aL_train,1);
    num_tL = size(tL_test,1) + size(tL_train,1);
    num_aR = size(aR_test,1) + size(aR_train,1);
    num_tR = size(tR_test,1) + size(tR_train,1);
    aL_test = [aL_test;aL_train];
    tL_test = [tL_test;tL_train];
    aR_test = [aR_test;aR_train];
    tR_test = [tR_test;tR_train];
    ...
    for idx = 1:num_tL
        % left data
        data_source{1}{num_aL+idx} = squeeze(tL_test(idx,:,:));
        tL_tmp = squeeze(tL_test(idx,:,:));
        tL_cov = cov(tL_tmp.');
        sigma1 = sigma1 + tL_cov/trace(tL_cov);
        % right data
        data_source{2}{num_aR+idx} = squeeze(tR_test(idx,:,:));
        tR_tmp = squeeze(tR_test(idx,:,:));
        tR_cov = cov(tR_tmp.');
        sigma2 = sigma2 + tR_cov/trace(tR_cov);
    end
    lambda1 = cal_shrinkage_modified([aL_test; tL_test]);
    lambda2 = cal_shrinkage_modified([aR_test; tR_test]);
