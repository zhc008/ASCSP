function [result,store_idx] = new_update_test4(gm,data_source,data_target)
csp_per_class=3;
result=gm;
...
for i = 1:2*n_target
    ...
    for j = (i+1):n_target*2
        ...
        mappedIdxj = ceil(j/2) + mod(j+1,2)*n_target;
        [X_LDA, predicted_y_class2] = lda_apply(Xproj_tmp(n_source*2+mappedIdxj,:), W, B);
           
        if predicted_y_class2 == -1 && X_LDA<-0.005
            continue;
        end
