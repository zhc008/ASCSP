...
%         end
    ...
    for i = [1:j-1 j+1:5]
        ...
        main_source = data_sources{combine_idx};
        main_size = size(main_source{1},2);
        for k = [1:combine_idx-1 combine_idx+1:5]
            if (k == i||k == j)
                continue;
            end
        ...
        %% training CSP using new covariance matrix. data_source is useless
        [ csp_coeff2,all_coeff2] = csp_analysis(data_sources{i},9,csp_per_class, 0,gms{i});
        [ data_source_filter2 ] = csp_filtering(data_sources{i}, csp_coeff2);
        ...
        %% Apply CSP in target subject
        [ data_target_filter2 ] = csp_filtering(main_source, csp_coeff2);
