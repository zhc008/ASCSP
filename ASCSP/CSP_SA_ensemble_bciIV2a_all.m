...
%     X1_s = X1_s(1:26,:,:);
    ...
    num_tmp = size(X1_s,1);
    x1_index = find(y_test_t == 1);
    x2_index = find(y_test_t == 2);
    channels = size(X_test_t, 2);
    ...
    for idx = 1:length(x1_index)
        % left data
        data_source{1}{idx+num_tmp} = squeeze(X_test_t(x1_index(idx),:,:));
        x1_tmp = squeeze(X_test_t(x1_index(idx),:,:));
        x1_cov = cov(x1_tmp.');
        sigma1 = sigma1 + x1_cov/trace(x1_cov);
    end
    ...
    for idx = 1:length(x2_index)
        % left data
        data_source{2}{idx+num_tmp} = squeeze(X_test_t(x2_index(idx),:,:));
        x2_tmp = squeeze(X_test_t(x2_index(idx),:,:));
        x2_cov = cov(x2_tmp.');
        sigma2 = sigma2 + x2_cov/trace(x2_cov);
    end
    
    cov_s1 = sigma1/(num_tmp + length(x1_index));
    cov_s2 = sigma2/(num_tmp + length(x2_index));
...
for j = 1:9
    ...
    for idx = 1:length(x1_index)
        % left data
        data_source{1}{idx} = squeeze(X_test_t(x1_index(idx),:,:));
    end
    
    for idx = 1:length(x2_index)
        % left data
        data_source{2}{idx} = squeeze(X_test_t(x2_index(idx),:,:));
    end
