...
tmp_std = zeros(12,1);
for j = 1:12
    tmp_std(j) = std(nonzeros(tmp_acc_all(:,j)));
end
mean_std = mean(tmp_std);
for j = 1:12
    if tmp_std(j) > mean_std
        new_source = cell(1,2);
        new_gm = cell(1,2);
        sigma1 = zeros(channels, channels);
        sigma2 = zeros(channels, channels);
        ...
        %% Train LDA. ASCSP with subspace alignment
        trainY = [(-1)*ones(size(data_source_log3{1},2),1); ones(size(data_source_log3{2},2),1)];
        size_xproj3 = size(data_source_log3{1},2) + size(data_source_log3{2},2);
        [W3,B3,~] = lda_train_reg(Xproj3(1:size_xproj3,:), trainY, 0);
