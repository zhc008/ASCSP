...
acc_for_all = zeros(24,10);
% acc_csp = zeros(12,10);

csp_per_class = 3;

gms = cell(10,24);
data_sources = cell(10,24);

weightings_i = zeros(12,24,10);
weightings_var = zeros(12,24);
...
%     alpha1 = mean(diag(sigma1));
%     alpha2 = mean(diag(sigma2));
%     gm{1} = lambda1*alpha1*eye(channels) + (1-lambda1)*sigma1;
%     gm{2} = lambda2*alpha2*eye(channels) + (1-lambda2)*sigma2;
    gm{1} = sigma1;
    gm{2} = sigma2;
    gms{instance,i} = gm;
...
for i = 1:12
    ...
    data_source = cell(1,2); %1 left 2 right
    channels = size(aL_test, 2);
    sigma1 = zeros(channels, channels);
    sigma2 = zeros(channels, channels);

    for idx = 1:num_aL_ol
        % left data
        data_source{1}{idx} = squeeze(aL_test(idx,:,:));
        aL_tmp = squeeze(aL_test(idx,:,:));
        aL_cov = cov(aL_tmp.');
        sigma1 = sigma1 + aL_cov/trace(aL_cov);
        % right data
        data_source{2}{idx} = squeeze(aR_test(idx,:,:));
        aR_tmp = squeeze(aR_test(idx,:,:));
        aR_cov = cov(aR_tmp.');
        sigma2 = sigma2 + aR_cov/trace(aR_cov);
    end
    for idx = 1:num_tL_ol
        % left data
        data_source{1}{num_aL_ol+idx} = squeeze(tL_test(idx,:,:));
        tL_tmp = squeeze(tL_test(idx,:,:));
        tL_cov = cov(tL_tmp.');
        sigma1 = sigma1 + tL_cov/trace(tL_cov);
        % right data
        data_source{2}{num_aR_ol+idx} = squeeze(tR_test(idx,:,:));
        tR_tmp = squeeze(tR_test(idx,:,:));
        tR_cov = cov(tR_tmp.');
        sigma2 = sigma2 + tR_cov/trace(tR_cov);
    end
%     lambda1 = cal_shrinkage_modified([aL_train; tL_train]);
%     lambda2 = cal_shrinkage_modified([aR_train; tR_train]);
    ...
    sigma1 = sigma1/(num_aL_ol+num_tL_ol);
    sigma2 = sigma2/(num_aR_ol+num_tR_ol);
    gm = cell(1,2); %cm 1 left 2 right 
%     alpha1 = mean(diag(sigma1));
%     alpha2 = mean(diag(sigma2));
%     gm{1} = lambda1*alpha1*eye(channels) + (1-lambda1)*sigma1;
%     gm{2} = lambda2*alpha2*eye(channels) + (1-lambda2)*sigma2;
    gm{1} = sigma1;
    gm{2} = sigma2;
    gms{instance,i+12} = gm;
    data_sources{instance,i+12} = data_source;
end
end
...
%         tmp_acc_all(i,j) = tmp_acc;
        ...
        if i > 12
            weightings_i(i-12,j+12,instance) = tmp_acc;
        else
            weightings_i(i,j,instance) = tmp_acc;
        end
    end
...
weightings = mean(weightings_i,3);
for i = 1:12
    for j = 1:24
        weightings_var(i,j) = var(weightings_i(i,j,:));
    end
end
