...
for instance = 1:10
for j = 1:12
    name1 = ['C:/Users/CZN/Documents/Balanced_mahta_data/S',num2str(j),'_I',num2str(instance),'_onln.mat'];
    load(name1)
    ...
    [X_LDA1, predicted_y_class1] = lda_apply([cell2mat(data_target_log{1})';cell2mat(data_target_log{2})'], W, B);
    ...
    [X_LDA2, predicted_y_class2] = lda_apply([cell2mat(data_target_log{1})';cell2mat(data_target_log{2})'], W, B);
    ...
    for i = [1:j-1 j+1:12]
        %% training CSP using new covariance matrix. data_source is useless
        [ csp_coeff,all_coeff] = csp_analysis(data_sources{instance,i},9,csp_per_class, 0,gms_r{instance,i});
        ...
        [X_LDA, predicted_y_class] = lda_apply([cell2mat(data_target_log{1})';cell2mat(data_target_log{2})'], W, B);
        X_LDAs = cell(1,12);
        ...
        for k = 1:12
            ...
            %% training CSP using new covariance matrix. data_source is useless
            [ csp_coeff2,all_coeff2] = csp_analysis(data_sources{instance,k},9,csp_per_class, 0,gms_r{instance,i});
            ...
            [X_LDA, predicted_y_class4] = lda_apply([cell2mat(data_target_log2{1})';cell2mat(data_target_log2{2})'], W2, B2);
            X_LDAs{k} = X_LDA;
...
%     predicted_y_vote = predicted_y_class1+predicted_y_class2+predicted_y_class3;
    predicted_y_vote = predicted_y_class1+predicted_y_class2;
    for index = 1:size(y_test_t, 1)
        if predicted_y_vote(index) == 0
            predicted_y_vote(index) = X_LDA1(index) + X_LDA2(index);
        end
    end
