...
for i = 1:12
    ...
    name2 = ['../Mahta_data/S',int2str(i),'_online.mat'];
    load(name2)
    ...
    [~, predicted_y_class1] = lda_apply(cell2mat(data_target_log{1})', W, B);
    predicted_y_class1(predicted_y_class1 == 1) = 0;   % incorrect choice
    predicted_y_class1(predicted_y_class1 == -1) = 1;
    [~, predicted_y_class2] = lda_apply(cell2mat(data_target_log{2})', W, B);    % there is a vector output! should all be -1!
    ...
    text = ['s', int2str(i),' to ','s', int2str(i)];
    fprintf(text)
    fprintf(': %f\n', acc2)
    acc_for_all(i) = acc2;
end
