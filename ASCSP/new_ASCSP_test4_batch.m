...
%     gm{2} = cov_s2 + c*eye(size(X1_s,2));
    ...
    for j = 1:5
        ...
        batch_size = 10;
        batch_num = floor(size(X1_t,1)/batch_size);
        acc_trials = 0;
        for k = 1:batch_num
            if k ~= batch_num
                X1_t_batch = X1_t(batch_size*(k-1)+1:batch_size*k,:,:);
                X2_t_batch = X2_t(batch_size*(k-1)+1:batch_size*k,:,:);
            else
                X1_t_batch = X1_t(batch_size*(k-1)+1:end,:,:);
                X2_t_batch = X2_t(batch_size*(k-1)+1:end,:,:);
            end
            % number of trials in one class
            num_tmp = size(X1_t_batch,1);
            data_target = cell(1,2);
            for idx = 1:num_tmp
            
                data_target{1}{idx} = squeeze(X1_t_batch(idx,:,:));
            
            end
        
            for idx = 1:num_tmp
            
                data_target{2}{idx} = squeeze(X2_t_batch(idx,:,:));
            
            end
        

            [gm1,store_idx] = new_update_test3(gm,data_source,data_target);
            added_trial(i,j) = added_trial(i,j) + size(store_idx,1);
%             target_trial(i,j) = size(X1_t,1);
            ...
            acc_trials = acc_trials + sum(temp);
            batch_acc = sum(temp)/length(temp); % this is the percent correct classification 
        end
        
        acc2 = acc_trials/2/size(X1_t,1);
