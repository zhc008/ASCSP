...
%         end
    ...
    [~, predicted_y_class3] = lda_apply([cell2mat(data_target_log3{1})';cell2mat(data_target_log3{2})'], W3, B3);
    test_err3 = sum(abs(y_test_t - predicted_y_class3))/size(y_test_t, 1)/2;
    acc3 = 1 - test_err3;
    text = ['s', int2str(j)];
    fprintf(text)
    fprintf(': %f\n', acc3)
