...
for i = 1:5 %loop through the 5 subjects
   ...
   for j = 1:5
       ...
       load(name1_t)
       load(name2_t)
       [X1_t, X2_t, X_test_t, ~] = extract_data(mrk, cnt, test_idx, true_y);
       f_X1_t = features(W, X1_t);
       f_X2_t = features(W, X2_t);
       y_test_t = ones(size(f_X1_t, 1)+size(f_X2_t, 1),1);
       y_test_t((size(f_X1_t,1)+1):end) = y_test_t((size(f_X1_t,1)+1):end)*2;
       f_X_t = [f_X1_t; f_X2_t];
